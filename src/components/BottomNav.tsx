...
export function BottomNav() {
  ...
  function renderItem(item: (typeof LEFT_ITEMS)[number] | (typeof RIGHT_ITEMS)[number]) {
    ...
    return (
      ...
      >
        ...
        {item.label}
        {item.badge && count > 0 && (
          <span className="absolute top-1 right-[calc(50%-16px)] min-w-[14px] h-[14px] rounded-full bg-accent text-white text-[9px] font-bold flex items-center justify-center px-0.5">
            {count > 9 ? "9+" : count}
          </span>
        )}
      </Link>
  ...
  return (
    ...
    >
      ...
      >
        <span className="accent-pill flex h-10 w-10 items-center justify-center -translate-y-1 shadow transition active:scale-90">
